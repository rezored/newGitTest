...
DHT _dhtSensor0(EXT_GROVE_D0, DHT22);
DHT _dhtSensor1(EXT_GROVE_D1, DHT22);
...
void publishDHTSensorData(bool isForceSend)
{
  ...
  if (millis() > _mesureTimeout || isForceSend)
  {
    ...
    if (_dht[1][0] != humidity1 || isForceSend)
    {
      ...
      strConcatenate(_topic, 5, MAIN_TOPIC, TOPIC_SEPARATOR, "1", TOPIC_SEPARATOR, HUMIDITY_SENSOR); // kmp/prodinowifi/0/humidity
      publish(_topic, _payload);
    }
    ...
    if (_dht[1][1] != temperature1 || isForceSend)
    {
      ...
      strConcatenate(_topic, 5, MAIN_TOPIC, TOPIC_SEPARATOR, "1", TOPIC_SEPARATOR, TEMPERATURE_SENSOR); // kmp/prodinowifi/0/temperature
      publish(_topic, _payload);
    }
